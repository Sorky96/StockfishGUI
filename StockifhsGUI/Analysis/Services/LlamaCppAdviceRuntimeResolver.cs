...
{
    private const string PreferredModelBaseName = "stockifhsgui-advice";
    private static readonly string[] PreferredModelFileNames =
    [
        "stockifhsgui-advice.gguf",
        "stockifhsgui-advice-q4_k_m.gguf",
    ...
    {
        string? fromEnvironment = Normalize(Environment.GetEnvironmentVariable("STOCKIFHSGUI_LLAMA_CPP_CLI_PATH"));
    ...
    {
        string? fromEnvironment = Normalize(Environment.GetEnvironmentVariable("STOCKIFHSGUI_LLAMA_CPP_MODEL_PATH"));
        if (File.Exists(fromEnvironment))
        {
            return fromEnvironment;
        }
