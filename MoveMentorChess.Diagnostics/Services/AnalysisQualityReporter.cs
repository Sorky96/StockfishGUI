...
public static class AnalysisQualityReporter
{
    ...
    public static AnalysisQualityReport Build()
        => Build(AnalysisStoreProvider.GetStore());

    public static AnalysisQualityReport Build(IAdviceFeedbackStore? feedbackStore)
    {
        ...
        IReadOnlyList<MoveAdviceFeedback> manualFeedback = feedbackStore?.ListMoveAdviceFeedback(limit: 200_000) ?? [];
    ...
    public static void RunReport()
        => RunReport(AnalysisStoreProvider.GetStore());

    public static void RunReport(IAdviceFeedbackStore? feedbackStore)
    {
        Console.WriteLine("=== Analysis Quality Report ===");
        Console.WriteLine();

        AnalysisQualityReport report = Build(feedbackStore);
