...
using StockifhsGUI.Avalonia.Views;
...
public sealed class MainWindowViewModel : ViewModelBase, IDisposable
{
    ...
    private string statusMessage = "Avalonia frontend is ready.";
    private string importedGameSummary = "Imported moves: none";
    private string suggestionText = "Stockfish suggestions: unavailable";
    ...
    public AnalysisWindow? CreateAnalysisWindow()
    {
        ...
        if (engine is null)
        {
            StatusMessage = "Stockfish is unavailable for analysis.";
            return null;
        }
    ...
    private void TryInitializeEngine()
    {
        ...
        try
        {
            ...
            {
                throw new FileNotFoundException("Could not locate stockfish.exe for the Avalonia frontend.");
            }

            engine = new StockfishEngine(enginePath);
            engine.SendCommand("setoption name MultiPV value 3");
            StatusMessage = $"Avalonia frontend is ready. Stockfish engine loaded from {enginePath}.";
        }
        catch (Exception ex)
        {
            engine = null;
            StatusMessage = $"Avalonia frontend is ready, but Stockfish is unavailable. {ex.Message}";
        }
    }
    ...
    private void RefreshEngineSummary()
    {
        if (engine is null)
        {
            SuggestionText = "Stockfish suggestions: unavailable";
    ...
    private static string? ResolveStockfishPath()
    {
        ...
        [
            ...
            Path.Combine(baseDirectory, "..", "..", "..", "..", "StockifhsGUI", "bin", "Debug", "net8.0-windows", "stockfish.exe"),
            Path.Combine(baseDirectory, "..", "..", "..", "..", "..", "StockifhsGUI", "bin", "Debug", "net8.0-windows", "stockfish.exe"),
            Path.Combine(Environment.CurrentDirectory, "StockifhsGUI", "bin", "Debug", "net8.0-windows", "stockfish.exe"),
