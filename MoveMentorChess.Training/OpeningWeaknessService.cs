...
public sealed class OpeningWeaknessService
{
    ...
    public OpeningWeaknessService(IAnalysisStore analysisStore)
        : this(new TrainingAnalysisDataSource(analysisStore, analysisStore), OpeningTheorySourceResolver.Create(analysisStore))
    {
    }

    public OpeningWeaknessService(IAnalysisResultStore resultStore)
        : this(
            resultStore,
            RequireMoveAnalysisStore(resultStore),
            resultStore as IOpeningTheoryStore)
    {
    }

    public OpeningWeaknessService(
    ...
    internal OpeningWeaknessService(TrainingAnalysisDataSource analysisDataSource, IOpeningTheoryStore? openingTheoryStore = null)
        : this(
            analysisDataSource,
            openingTheoryStore is null ? null : new OpeningTheoryQueryService(openingTheoryStore))
    {
    ...
    internal OpeningWeaknessService(TrainingAnalysisDataSource analysisDataSource, OpeningTheoryQueryService? openingTheory)
    {
    ...
    {
        StoredMoveContext played = candidate.Move.Move;
        StoredMoveAdviceContext advice = candidate.Move.Advice;
        string betterMove = TryFormatTheoryMove(played.FenBefore)
            ?? FormatBestMove(played.FenBefore, played.BestMoveUci);
